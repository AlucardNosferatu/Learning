...
def make_discriminator_model(is_training):
...
def make_generator_model(is_training):
    model = tf.keras.Sequential()
    model.add(DenseLayer(1024))
    model.add(BatchNorm(is_training=is_training))
...
class CGAN:
    def __init__(self, args):
        ...
        self.g = make_generator_model(is_training=True)
        self.d = make_discriminator_model(is_training=True)
        self.g_optimizer = tf.keras.optimizers.Adam(lr=5 * self.learnning_rate, beta_1=0.5)
        self.d_optimizer = tf.keras.optimizers.Adam(lr=self.learnning_rate, beta_1=0.5)
    ...
    def train(self, load=False):
        ...
        # if you want to load a checkpoints,set load flags to be true
        if load:
            ...
            for batch_images, batch_labels in self.datasets:
                ...
                if step % 50 == 0:
                    ...
                    save_images(
                        ...
                        ) + '/' + self.model_name + '_train_{:02d}_{:04d}.png'.format(
                            epoch,
                            int(step)
                        )
                    )
